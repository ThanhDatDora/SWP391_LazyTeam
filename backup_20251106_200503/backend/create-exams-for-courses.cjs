  ...
  try {
    ...
    for (const mooc of moocsResult.recordset) {
      ...
      const existingExam = await pool.request()
        .input('mooc_id', sqlLib.BigInt, mooc.mooc_id)
      ...
      const examResult = await pool.request()
        .input('mooc_id', sqlLib.BigInt, mooc.mooc_id)
        .input('name', sqlLib.NVarChar(255), `${mooc.course_title} - Final Exam`)
        .input('duration_minutes', sqlLib.Int, 45)
        .input('attempts_allowed', sqlLib.Int, 3)
        .query(`
      ...
      const questionsResult = await pool.request()
        .input('course_id', sqlLib.Int, mooc.course_id)
        .query(`
      ...
      for (const question of questionsResult.recordset) {
        ...
        await pool.request()
          .input('exam_id', sqlLib.BigInt, examId)
          .input('question_id', sqlLib.BigInt, question.question_id)
          .input('order_no', sqlLib.Int, orderNo)
          .input('points', sqlLib.Decimal(5, 2), points)
          .query(`
