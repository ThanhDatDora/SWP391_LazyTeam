    ...
    for (const mooc of moocs.recordset) {
      ...
      for (const q of questions) {
        // Insert question
          ...
          .input('mooc_id', sqlLib.BigInt, moocId)
          .input('stem', sqlLib.NVarChar(sqlLib.MAX), q.stem)
          .input('qtype', sqlLib.NVarChar(50), q.qtype)
          .input('difficulty', sqlLib.NVarChar(20), q.difficulty)
          .input('max_score', sqlLib.Decimal(5, 2), q.max_score)
          .input('created_by', sqlLib.BigInt, 2) // Instructor
          .query(`
        ...
        for (const opt of q.options) {
          await pool.request()
            .input('question_id', sqlLib.BigInt, questionId)
            .input('label', sqlLib.NVarChar(10), opt.label)
            .input('content', sqlLib.NVarChar(sqlLib.MAX), opt.content)
            .input('is_correct', sqlLib.Bit, opt.is_correct)
            .query(`
